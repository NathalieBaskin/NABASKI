...
function KundGalleri() {
  ...
  const fetchComments = async (image) => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/comments/${encodeURIComponent(image)}`
      );
      const data = await res.json();
  ...
  const handleLike = async (image) => {
    try {
      const res = await fetch(
        `http://localhost:5000/api/like/${encodeURIComponent(image)}`,
        { method: "POST" }
      );
      const data = await res.json();
      setLikes((prev) => ({ ...prev, [image]: data.likes }));
  ...
  const handleCommentSubmit = async (image) => {
    ...
    try {
      const res = await fetch(
        `http://localhost:5000/api/comment/${encodeURIComponent(image)}`,
        {
          method: "POST",
            ...
            <div key={image} className="image-card">
              ...
              <div className="image-icons">
                <span onClick={() => handleLike(image)}>
                  <FontAwesomeIcon
                    icon={likes[image] > 0 ? solidHeart : regularHeart}
                  />
      ...
      {selectedImageIndex !== null && (
        <div className="slideshow">
          ...
          <div className="slideshow-icons">
            <span
              onClick={() =>
                handleLike(selectedGallery.images[selectedImageIndex])
              }
            >
              <FontAwesomeIcon
                icon={
                  likes[selectedGallery.images[selectedImageIndex]] > 0
                    ? solidHeart
                    : regularHeart
                }
              />
          ...
          <div className="comment-section">
            ...
            <button
              onClick={() =>
                handleCommentSubmit(selectedGallery.images[selectedImageIndex])
              }
            >
              Kommentera
            </button>
            <div className="comments">
              {comments[selectedGallery.images[selectedImageIndex]] &&
                comments[selectedGallery.images[selectedImageIndex]].map(
                  (comment, index) => (
                    <p key={index}>
